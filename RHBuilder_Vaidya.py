...
class RoleHierarchy:
    """
    Build a Role Hierarchy (RH) from a deployed set of roles -> permissions.
    Produces the minimal-edge complete hierarchy (transitive reduction).
    """

    def __init__(self, roles_perms: dict[int, set]):
        # Filter empty roles (disallowed) and normalize copies
        ...
        # Edge set (parent, child) mirrors children map
        self.E = set()

    def build(self) -> dict[int, set]:
        """Run RH-Builder over all roles (order-insensitive)."""
        for r in self.roles:
            if r != self.SUPER_ROLE:
                self._add_edge(self.SUPER_ROLE, r)

        # for each r in DROLES call rh_builder_iteration
