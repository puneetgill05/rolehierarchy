...
from RBAC_to_RH import check_RH, get_metrics, roles_edges_to_dict, reconstruct_roles_from_RH, rbac_edges_to_dict, \
    roles_dict_to_edges, remove_redundant_roles
...
def rbac_to_rh(upfilename, roles):
    ...
    RH_mapped_prime = remove_redundant_roles(RH_mapped)
    if check_RH(RH_mapped_prime, up, usermap, permmap):
        RH_mapped = RH_mapped_prime
...
def run_rbac_to_rh_no_new_roles(upfilename: str, roles: dict):
    ...
    RH_flat_G = dict_to_digraph(RH_flat)
    print(f'RH is DAG? {nx.is_directed_acyclic_graph(RH_flat_G)}')
    cycles = list(nx.simple_cycles(RH_flat_G))
    print("Cycles found:")
    for cycle in cycles:
        print(cycle)

    get_metrics(RH_flat, run_random_walk=False)
    ...
    metrics = get_metrics(RH, run_random_walk=True, use_sampling=True)
    # metrics = dict()
    return RH, metrics
