...
from build_chains import pretty_print_chains
...
def get_user_chain_OLD(roles_users: dict):
...
def get_user_chain(roles_users: dict):
    chains = pretty_print_chains(list(roles_users.values()))
    subsets_users = []
    for chain in chains:
        sub = list()
        for element in chain:
            for r in roles_users:
                if element == roles_users[r]:
                    sub.append(r)
        subsets_users.append(sub)
    print('Subset users:')
    #pprint(subsets_users)
...
def get_perm_chain(roles_perms: dict):
    chains = pretty_print_chains(list(roles_perms.values()))
    subsets_perms = []
    for chain in chains:
        sub = list()
        for element in chain:
            for r in roles_perms:
                if element == roles_perms[r]:
                    sub.append(r)
...
def get_perm_chain_OLD(roles_perms: dict):
...
def remove_redundant_role_role_edges(roles: dict, RHorig: dict, up: dict, usermap: dict, permmap: dict):
    RH = copy.deepcopy(RHorig)
...
def remove_redundant_edges(RHorig: dict) -> dict:
    RH = copy.deepcopy(RHorig)
...
def remove_edges_if_common_perms(RHorig: dict):
    RH = copy.deepcopy(RHorig)

    print("REMOVING EDGES IF COMMON")
...
def rbac_to_rh(roles_as_edges: dict, usermap: dict, permmap: dict):
    ...
    max_user_chain_size = 0
    max_perm_chain_size = 0
    for chain in subsets_users:
        if len(chain) > max_user_chain_size:
            max_user_chain_size = len(chain)
    for chain in subsets_perms:
        if len(chain) > max_perm_chain_size:
            max_perm_chain_size = len(chain)

    # if len(subsets_users) >= len(subsets_perms):
    if max_user_chain_size >= max_perm_chain_size:
...
def reconstruct_roles_from_RH(RH: dict):
    ...
    for r in RH:
        ...
        #     recon_roles[r] = set()
        users_r = users_r.union(inherited_users[r])
        perms_r = perms_r.union(inherited_perms[r])
...
def remove_redundant_roles(RHorig: dict):
    RH = copy.deepcopy(RHorig)
    print("REMOVING REDUNDANT ROLES")
    recon_roles = reconstruct_roles_from_RH(RH)
    ...
    for r in duplicate_roles:
        for dup in duplicate_roles[r]:
            if dup in RH and dup not in duplicate_roles:
                RH.pop(dup)
...
def rbac_to_rh_no_new_roles(upfilename: str, roles: dict, edge_threshold=10):
    ...
    # get_metrics(RH, use_sampling=True, run_random_walk=False)
    ...
    RH_prime = remove_redundant_roles(RH)
    if check_RH(RH_prime, up, usermap, permmap):
        RH = RH_prime
    # RH = remove_redundant_role_role_edges(roles, RH, up, usermap, permmap)
    print('==============================')
    print(f'Running: {upfilename}')
    print('==============================')

    # RH_prime = remove_edges_if_common_perms(RH)
    # if check_RH(RH_prime, up, usermap, permmap):
    #     RH = RH_prime
    RH_prime = remove_redundant_edges(RH)
    if check_RH(RH_prime, up, usermap, permmap):
        RH = RH_prime
    get_metrics(RH, run_random_walk=False)

    # RH_prime = remove_more_edges(RH, up, usermap, permmap)
    # if check_RH(RH_prime, up, usermap, permmap):
    #     RH = RH_prime
    ...
    num_trials = int(k ** 2 * np.log(num_edges - num_vertices))
    # num_trials = 200
    # print(f'# trials: k^2: {k ** 2}, E: {num_edges}, V: {num_vertices}, = {num_trials}')
    # for i in range(num_trials):
    #     newRH = minimize_edges_subgraph(RH, up, usermap, permmap, k=k)
    #     check = check_RH(newRH, up, usermap, permmap)
    #     if check:
    #         RH = newRH

    RH_prime = minimize_edges(RH, up, usermap, permmap, edges_to_keep=set())
    # if check_RH(RH_prime, up, usermap, permmap):
    #     RH = RH_prime
...
def minimize_edges_subgraph(RH: dict, up: dict, usermap: dict, permmap: dict, k=10, persolve_time=86400):
    print("REMOVING MINIMIZE EDGES SUBGRAPH")

    inv_permmap = {v: kk for kk, v in permmap.items()}
    inv_usermap = {v: kk for kk, v in usermap.items()}
    k = min(len(RH), k)
    random_roles = random.sample(list(RH.keys()), k)
    subRH = dict()
    roles_removed = dict()
    for r in random_roles:
        roles_removed[r] = set()
